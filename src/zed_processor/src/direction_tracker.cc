...
/*   /1* for (int i = 0; i < target_->keypoints.size(); ++i) { *1/ */
/*   /1*   const auto& target_kp = target_->keypoints[i]; *1/ */
/*   /1*   cv::Mat target_desc = target_->descriptors.row(i); *1/ */

/*   /1*   double d1 = 100000, d2 = 100000; *1/ */
/*   /1*   int j1 = -1; *1/ */

/*   /1*   for (int j = 0; j < keypoints.size(); ++j) { *1/ */
/*   /1*     double d = descriptorDist(target_desc, descriptors.row(j)); *1/ */
/*   /1*     if (d < d1) { *1/ */
/*   /1*       d2 = d1; *1/ */
/*   /1*       d1 = d; *1/ */
/*   /1*       j1 = j; *1/ */
...
/*   /1*   if (d1 < d2 - 10) { *1/ */
/*   /1*     reprojection_features_.push_back( *1/ */
/*   /1*       MonoReprojectionFeature { *1/ */
/*   /1*         unproject_(keypoints[j1]), *1/ */
/*   /1*         unproject_(target_kp), *1/ */
/*   /1*         keypoints[j1], *1/ */
/*   /1*         target_kp, *1/ */ 
/*   /1*       } *1/ */
/*   /1*     ); *1/ */
